...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            respond("Token expired", 401, response);
            return;
        } catch (SignatureVerificationException signatureVerificationException) {
            respond("Token not valid", 403, response);
            return;
        }
...
//                List<Role> roles = ContextBridge.services().getRoleService().findRolesByUserEmail(user);

//                roles.forEach(role -> {
//                    authorities.add(new AuthorityRole(ROLE_PREFIX + role.getName()));
//                });
...
//                return new UsernamePasswordAuthenticationToken(user, null, authorities);
    ...
    private void respond(String message, int code, HttpServletResponse response) throws IOException {
